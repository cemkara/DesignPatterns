...
namespace ChainofResponsibility.ChainofResponsibility
{
    public class ManagerAsistant : Employee
    {
        public override void HandleRequest(CustomerProcessViewModel request)
        {
            ...
            if (request.Ammount <= 150000)
            {
                //ödemeyi bankanın en düşük görevlisi yapar.  
                customerProcess.Description = "Yönetici Asistanı: Para çekme işlemi başarıyla gerçekleşti.";
                dbContext.CustomerProcesses.Add(customerProcess);
                dbContext.SaveChanges();
            }
            else if (nextHandler != null)
            {
                customerProcess.Description = "Yönetici Asistanı: Yetki hatası, para çekme işlemi başarısız. Yöneticiye yönlendirildi.";
                dbContext.CustomerProcesses.Add(customerProcess);
                dbContext.SaveChanges();
